...
export const handler = async (event) => {
  ...
  try {
    ...
    console.log('Google Veo3 Fast Preview video generation request:', {
      projectId,
      videoId,
    ...
    const finalNegativePrompt = negativePrompt || 'low quality, blurry, distorted, complex effects, excessive movement';
    
    console.log('Final prompt to Veo3 Fast Preview:', finalPrompt);
...
${finalNegativePrompt ? `Negative Prompt (things to avoid): ${finalNegativePrompt}` : ''}`
    ...
    const { error: updateError } = await supabaseAdmin
      ...
      .update({
        ...
        metadata: {
          ...
          model: 'veo-3.0-fast-preview-001',
          imageUrl: imageUrl,
    ...
    pollVeo3FastOperation(genAI, generationId, videoId, supabaseAdmin, user.sub).catch(console.error);
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        status: 'processing',
        message: 'Google Veo3 Fast Preview 비디오 생성이 시작되었습니다. 약 30초-1분 소요됩니다.',
        model: 'veo-3.0-fast-preview-001'
      })
    };

  } catch (error) {
    console.error("Veo3 Fast Preview Video Generation Error:", error);
...
// 백그라운드에서 Veo3 Fast 작업 상태 폴링
async function pollVeo3FastOperation(genAI, generationId, videoId, supabaseAdmin, userId) {
  let attempts = 0;
  const maxAttempts = 12; // 최대 1분 (5초 * 12) - Fast는 매우 빠름
  
  while (attempts < maxAttempts) {
    await new Promise((resolve) => setTimeout(resolve, 5000)); // 5초 대기
    
    try {
      console.log(`Polling attempt ${attempts + 1} for Veo3 Fast Preview generation`);
      ...
      // Veo3 Fast Preview는 매우 빠르게 생성됨
      if (attempts === 0) {
        // 실제 Veo3 Fast API는 비디오 URL을 직접 반환할 것임
        const videoUrl = `https://storage.googleapis.com/veo3-fast-generated/${generationId}.mp4`;
...
// Storage에 비디오 다운로드 (Veo2, Veo3와 동일한 함수 재사용)
async function downloadToStorage(videoUrl, videoId, userId, supabaseAdmin) {
  console.log('Auto-downloading Veo3 Fast video to Storage...');
