...
// 파일을 Google Drive에 백업 (OAuth)
async function backupFileToGoogleDriveOAuth(fileUrl, fileName, folderId) {
  try {
    console.log(`Backing up file: ${fileName} to Google Drive (OAuth)`);
...
async function backupUpscaleVideos(limit = 5) {
  ...
  for (const video of videos) {
    ...
    const result = await backupFileToGoogleDriveOAuth(
...
export const handler = async (event) => {
  ...
  try {
    ...
    console.log('=== GOOGLE DRIVE OAUTH BACKUP START ===');
    ...
    const summary = {
      success: true,
      message: `Google Drive OAuth backup completed (${type})`,
      type,
